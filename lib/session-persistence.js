...
class SessionPersistence {
  ...
  loadTodoList(todoListId) {
    return deepCopy(this._todoLists.find(todoList => todoList.id === todoListId));
  }
  ...
  toggleTodo(todoListId, todoId) {
    let originalTodoList = this._todoLists.find(todoList => todoListId === todoList.id);
    let originalTodo = originalTodoList.todos.find(todo => todoId === todo.id);
