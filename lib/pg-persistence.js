...
class PgPersistence {
  ...
  async loadTodoList(todoListId) {
    ...
    let todoListResult = dbQuery(TODOLIST, todoListId, this.username);
  ...
  async toggleTodo(todoListId, todoId) {
    ...
    await dbQuery(TOGGLE_DONE, todoListId, todoId, this.username);
  }
  ...
  async deleteTodo(todoListId, todoId) {
    ...
    await dbQuery(DELETE_TODO, todoListId, todoId, this.username);
  }
  ...
  async completeAllTodos(todoListId) {
    const COMPLETE_ALL_TODOS = "UPDATE todos SET done = true" + 
                               ...
                               "    AND username = $2" + 
                               "    AND NOT done";
    await dbQuery(COMPLETE_ALL_TODOS, todoListId, this.username);
  }
  ...
  async addTodoList(title) {
    const CREATE_TODOLIST = "INSERT INTO todolists (title, username)" +
                            "  VALUES ($1, $2)";
    await dbQuery(CREATE_TODOLIST, title, this.username);
  }
  ...
  async addTodo(todoListId, title) {
    const CREATE_TODO = "INSERT INTO todos (title, todolist_id, username)" + 
                        "  VALUES ($2, $1, $3)";
    await dbQuery(CREATE_TODO, todoListId, title, this.username);
  }
  
  // Set a new title for the specified todo list.
  async renameTodoList(todoListId, newTitle) {
    const RENAME_LIST = "UPDATE todolists SET title = $2" + 
                        "  WHERE id = $1 AND username = $3";
    await dbQuery(RENAME_LIST, todoListId, newTitle, this.username);
  }
  ...
  async existsTodoListTitle(newTodoListTitle) {
    const FIND_LIST_TITLE = "SELECT * FROM todolists" + 
                            "  WHERE title = $1 AND username = $2";
    let result = await dbQuery(FIND_LIST_TITLE, newTodoListTitle, this.username);
